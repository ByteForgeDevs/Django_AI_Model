...
```console
$ docker pull ghcr.io/byteforgedevs/djaudit:0.2.0
```

Tags follow the release: `0.2.0` is the version in `pyproject.toml`, and the
...
```console
$ docker run --rm -v "$PWD:/src:ro" ghcr.io/byteforgedevs/djaudit:0.2.0
```
...
```console
$ docker run --rm ghcr.io/byteforgedevs/djaudit:0.2.0 rules
$ docker run --rm ghcr.io/byteforgedevs/djaudit:0.2.0 explain DJP-001
$ docker run --rm -v "$PWD:/src:ro" ghcr.io/byteforgedevs/djaudit:0.2.0 run /src --family DJS
```
...
$ docker run --rm -v "$PWD:/src:ro" -v "$PWD/out:/out" \
    ghcr.io/byteforgedevs/djaudit:0.2.0 run /src --format sarif -o /out/djaudit.sarif
```
