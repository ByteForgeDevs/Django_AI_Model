...
class TestItInheritsNoSecrets:
    ...
    def test_bytecode_writing_is_off_so_the_audit_leaves_no_traces(self, tmp_path: Path) -> None:
        ...
        run_python(self_check(), ["-c", "import leftover"], cwd=tmp_path)
    ...
    def test_the_interpreter_runs_isolated(self, tmp_path: Path) -> None:
        """Belt and braces: `-I` means it would ignore them even if one got in."""
        outcome = probe(self_check(), "__import__('sys').flags.isolated", cwd=tmp_path)
        assert outcome.stdout.strip() == "1"

    def test_a_raw_command_is_not_forced_isolated(self, tmp_path: Path) -> None:
        """`run_command` runs what it is given; `-I` belongs to `run_python`."""
        outcome = run_command(
            [sys.executable, "-c", "import sys; print(sys.flags.isolated)"], cwd=tmp_path
        )
...
class TestDescribingWhatHappened:
    def test_a_success_names_the_code_and_the_time(self, tmp_path: Path) -> None:
        ...
        assert outcome.describe() == (
            f"`{executable} -I -c pass` exited 0 in {outcome.duration:.1f}s"
        )
    ...
    def test_a_failure_is_described_whole(self, tmp_path: Path) -> None:
        ...
        executable = self_check().executable
        assert outcome.describe() == (
            f"`{executable} -I -c raise SystemExit(2)` exited 2 in {outcome.duration:.1f}s"
        )
    ...
    def test_a_timeout_says_it_was_killed(self, tmp_path: Path) -> None:
        ...
        executable = self_check().executable
        assert outcome.describe() == (
            f"`{executable} -I -c import time; time.sleep(60)` was killed "
